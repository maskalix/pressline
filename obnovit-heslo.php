...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user) {
        ...
        // Store the token and expiration time in the database for later verification
        ...
        try {
            ...
            $mail->addAddress($user['mail'], $username);
            // Content
            $mail->isHTML(true);
            $mail->Subject = t('reset.request.email_subject', $name);
            $resetLink = $url . "/reset-hesla.php?token=$token";
            $mail->Body = t('reset.request.email_body', $url, $resetLink);
            $mail->CharSet = 'UTF-8';

            $mail->send();
            $success = t('reset.request.flash.sent');
        } catch (Exception $e) {
            error_log("PHPMailer error: " . $mail->ErrorInfo);
            $success = t('reset.request.flash.error');
        }
    } else {
        $success = t('reset.request.flash.sent');
    }
}
$sitename = t('reset.request.title');
?>
...
<body>
    <main>
        <div class="img" style="flex-direction:column;gap:var(--space-3);">
            ...
            <h1><?= te('reset.request.title') ?></h1>
            <p style="color:var(--text-muted);font-size:var(--text-sm);text-align:center;margin:0;">
                <?= te('reset.request.subtitle') ?>
            </p>
        </div>
        <div class="form">
            <div id="info">
                <?php if (isset($success) && $success !== '') echo esc($success); ?>
            </div>
            <form action="obnovit-heslo.php" method="post">
                <?= csrf_field() ?>
                <label for="username"><?= te('reset.request.username') ?></label>
                <input id="username" type="text" name="username" autocomplete="username" placeholder="<?= te('reset.request.placeholder') ?>" required>
                <input class="submit btn-green" type="submit" value="<?= te('reset.request.submit') ?>">
            </form>
            <div class="other-options">
                <a href="./login.php">
                    <span class="iconify-inline" data-icon="ic:round-arrow-back"></span><?= te('reset.request.back_login') ?>
                </a>
                <a href="<?= esc($frontend); ?>">
                    <span class="iconify-inline" data-icon="ic:round-home"></span><?= te('reset.request.home') ?>
                </a>
            </div>
        </div>
    </main>
